...
import yaml
from shapely import LineString, Point, BaseGeometry
...
def _parse_planets(
    c: dict,
) -> tuple[list[BaseGeometry], list[DynObstacleModel], list[NPAgent]]:
    planets = []
    satellites = []
    satellites_players = []
...
def sim_context_from_yaml(file_path: str):
    ...
    static_obstacles = planets.append(env_limits)

    static_obstacles: list[StaticObstacle] = [
        StaticObstacle(shape=s) for s in static_obstacles
    ]
    # players
    players = {playername: RocketAgent()}
    models = {playername: RocketModel.default(x0)}
    for i, (s, sagent) in enumerate(zip(satellites, satellites_npagents)):
        p = PlayerName("P" + str(i))
        players[p] = sagent
        models[p] = s
...
if __name__ == "__main__":
    config_file = "src/pdm4ar/exercises_def/ex09/config_planet.yaml"
