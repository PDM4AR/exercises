...
def ex2_get_expected_results() -> list[Optional[list[tuple[Path, OpenedNodes]]]]:
    ...
    # test grid 1 id
    expected_results[10] = [
        ([4, 3, 7, 11, 10, 9, 5], [4, 3, 7, 8, 11, 10, 9, 5]),
    ...
    # test grid 2 id
    # Branch-local visitation produces millions of entries; keep the exact sequences compressed.
    with gzip.open(pathlib.Path(__file__).with_name("expected_id_grid02.json.gz"), "rt", encoding="utf-8") as file:
        expected_results[14] = [(path, opened) for path, opened in json.load(file)]
