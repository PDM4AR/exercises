...
def randomize_reset_constraints(constraints: Constraints, p_constraints: Constraints, 
                                reset_values: Constraints
    ) -> Constraints:
    ...
    for name_constraint in Constraints.__annotations__.keys():
        ...
        x = np.random.choice([getattr(constraints, name_constraint), 
...
def milp_generator(milp_seed: int, optimization_cost: Optional[IntEnum] = None, 
    ...
    if optimization_cost is None:
        ...
        optimization_cost = OptimizationCost(np.random.choice(optimization_costs_enum_values, 1))
    ...
    max_distance_individual_sail = 0.8*(y_range[1]-y_range[0]) + 0.8*(x_range[1]-x_range[0]) + x_offset_levels
    ...
    min_fix_ship_individual_island = round(0.5*time_compass_range[0]+np.random.random()*(time_compass_range[1]-time_compass_range[0]))
    ...
    for k in range(n_arch):
        ...
        for _ in range(n_islands_arch_tmp):
            ...
            while pos_ok is not True:
                ...
                generated_island = island_generator(island_id, k, x_range,y_range,departure_range,arrival_range,time_compass_range_tmp,crew_range_tmp)
                pos_ok = not island_is_overlapping(generated_island, islands, island_radius_viz)
                n_trials_samples += 1
            islands.append(generated_island)
            island_id += 1
    islands = tuple(islands)

    constraints = Constraints(min_fix_ship_individual_island, 
                   min_total_crew, max_total_crew, max_duration_individual_sail, 
