...
class Viz:
    ...
    def display_text_report(self, r_viz, problem: ProblemVoyage, feasibility_score: int,
                            voyage_plan: Optional[List[int]], 
                            est_cost: Cost, gt_cost: Cost, cost_score: CostScore, 
                            timing: float, violations: Violations) -> None:
        ...
        for problem_data_name in problem.__annotations__.keys():
            ...
            if isinstance(problem_data, IntEnum):
                ...
                for constraint_name in problem_data.__annotations__.keys():
                    ...
                    text_problem += f"{constraint_name}: " + (f"{constraint:.2f}\n" 
                                    if isinstance(constraint, float) else f"{constraint}\n")
    ...
    def visualize(
        self, r_sec: Report, problem: ProblemVoyage, feasibility_score: int, gt_cost: Optional[Cost], 
        cost_score: Cost, voyage_plan: Optional[List[int]], est_cost: Optional[Cost],
        violations: Optional[Violations] = None, timing: Optional[float] = None
    ) -> None:
