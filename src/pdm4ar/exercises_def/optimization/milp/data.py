...
from pdm4ar.exercises_def.optimization.milp.structures import Island, PirateProblem1
...
def milp_generator(milp_type: Literal, milp_difficulty: Literal, milp_seed: int) -> PirateProblem1:
    ...
    if milp_type == MilpCase.island:
        ...
        island_id = 0
        islands = []
        x_range = [np.random.randint(-10,11), np.random.randint(15,36)]
                ...
                while pos_ok is not True:
                    if n_trials_samples >= max_trials_samples:
                        raise ValueError(f"Exceeded the amount of trials ({n_trials_samples}) to generate a non overlapping island."
                                        " Try to increase the x_range or y_range, or run again with a different seed.")
        ...
        problem = PirateProblem1(start_crew, islands, min_fix_ship_individual_island, 
                            min_crew, max_crew, max_duration_individual_sail, max_distance_individual_sail)

    elif milp_type == MilpCase.test:
