...
def get_local_queries(G: WeightedGraph, id: str) -> set[Query]:
    ...
    local_queries = check_local_queries(my_queries[id], G) if my_queries[id] else set()
...
def check_local_queries(local_queries: set[Query], G: WeightedGraph) -> set[Query]:
    """
    ...
    for q in invalid_queries:
        print(f"Query {q} does not exist in the graph. Removing it.")
