...
class SatellitePlanner:
    ...
    def compute_trajectory(
        self, init_state: SatelliteState, goal: PlanningGoal, X_guess, U_guess, p_guess
        ...
        while not converged and it < 10:
            print(f"====== new iteration {it}======")
    ...
    def _get_constraints(self) -> list[cvx.constraints]:
        ...
        # 0 <= F(t) <= F_max
        for k in range(K):
            constraints.append(self.variables["U"][0, k] >= self.sp.F_limits[0])
            constraints.append(self.variables["U"][0, k] <= self.sp.F_limits[1])
    ...
    def _get_objective(self) -> cvx.Problem:
        ...
        objective = (
            ...
            - self.toggle_params.weight_mf * self.variables["X"][5, self.params.K - 1]
    ...
    def _get_cmds_states(
        ...
        cmds_list = [SatelliteCommands(f, dp) for f, dp in zip(F, dphi)]
    ...
    def _extract_seq_from_array() -> tuple[DgSampledSequence[SatelliteCommands], DgSampledSequence[SatelliteState]]:
        ...
        cmds_list = [SatelliteCommands(l, r, dp) for l, r, dp in zip(F_l, F_r, dphi)]
        mycmds = DgSampledSequence[SatelliteCommands](timestamps=ts, values=cmds_list)
